<div>
    <div x-data="{ destroyLesson(lessonId) {
        Swal.fire({
            title: '¿Estás seguro?',
        ...
        Sortable.create($refs.lessonList, {
            group: 'lessons',
            animation: 150,
            handle: '.handle',
            onEnd: function (evt) {
                    ...
                    <div x-data="{ isOpen: false }" class="bg-white rounded-lg shadow-lg px-6 py-4 handle mb-4" style="cursor: move;">
                        @if ($lessonEdit['id'] == $lesson->id)
                            <form wire:submit.prevent="update">
                                <!-- Campos del formulario de edición con mensajes de error -->
                                <div class="flex items-center space-x-2">
                                    <x-label>Lección {{$loop->iteration}}:</x-label>
                                    <x-input wire:model="lessonEdit.name" class="flex-1" />
                                    <x-input-error for="lessonEdit.name" />
                                </div>
                                <!-- Otros campos de entrada -->
                                <!-- Campo de selección de plataforma y inputs de video/URL -->
                                <div class="mt-2">
                                    <x-label class="mb-1">Plataformas</x-label>
                                    <div class="md:flex md:items-center md:space-x-4 space-y-4 md:space-y-0">
                                        <button type="button" x-on:click="platform = 1" :class="{ 'border-indigo-500 text-indigo-500': platform == 1, 'border-gray-300': platform != 1 }" class="inline-flex flex-col justify-center items-center w-full md:w-20 h-24 border rounded">
                                            <i class="fas fa-video text-2xl"></i>
                                            <span class="text-sm mt-2">Video</span>
                                        </button>
                                        <button type="button" x-on:click="platform = 2" :class="{ 'border-indigo-500 text-indigo-500': platform == 2, 'border-gray-300': platform != 2 }" class="inline-flex flex-col justify-center items-center w-full md:w-20 h-24 border rounded">
                                            <i class="fab fa-youtube text-2xl"></i>
                                            <span class="text-sm mt-2">Youtube</span>
                                        </button>
                                    </div>
                                </div>
                                <!-- Mostrar los campos de carga o URL según la selección -->
                                <div class="mt-2" x-show="platform == 1">
                                ...
                                <div class="mt-2" x-show="platform == 2">
                        ...
                        @else
                            ...
                            <div x-show="isOpen" x-transition>
                                <p class="text-sm">Descripción: {{$lesson->description ?? 'Sin descripción'}}</p>
                                <p class="text-sm">Documento: 
                                    @if($lesson->document_path)
                                        <a href="{{ Storage::url($lesson->document_path) }}" target="_blank">{{ $lesson->document_original_name }}</a>
                                    @else
                                        No hay documento adjunto.
                                    @endif
                                </p>
                                <p class="text-sm">Video: 
                                    @if($lesson->platform == 1 && $lesson->video_path)
                                        <a href="{{ Storage::url($lesson->video_path) }}" target="_blank">{{ $lesson->video_original_name }}</a>
                                    @elseif($lesson->platform == 2)
                                        <a href="{{ $lesson->video_original_name }}" target="_blank">Ver en YouTube</a>
                                    @else
    ...
    <!-- Formulario para crear nueva lección -->
